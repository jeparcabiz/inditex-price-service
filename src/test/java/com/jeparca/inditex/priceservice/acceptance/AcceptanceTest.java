...
public class AcceptanceTest {
	...
	public void request_price() {
		...
		Response response = given().queryParams(queryParams).when().get(PRICES_V1_ENDPOINT);
	...
	private Map<String, Object> getQueryParams() {
		...
		if (this.applicationDate != null) {
			...
			queryParams.put(APPLICATION_DATE_QUERY_PARAM_NAME, applicationDate);
		}
		if (this.brandId != null) {
			queryParams.put(BRAND_ID_QUERY_PARAM_NAME, this.brandId);
		}
		if (this.productId != null) {
			queryParams.put(PRODUCT_ID_QUERY_PARAM_NAME, this.productId);
		}
		return queryParams;
	}
